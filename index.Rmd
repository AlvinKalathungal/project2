...
results %>% ggplot(aes(PC3, PC4, color=`Attack/sec`)) + geom_point(size=4)
```
...
data <- sample_frac(smiteData)  #randomly order rows
...
for (i in 1:k) {
    ...
    diags <- rbind(diags, class_diag(probs, truth, positive = "Physical"))
}
